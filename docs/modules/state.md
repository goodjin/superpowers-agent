...
- `pending_question` 只保存等待用户回答的问题；问题回答后必须清空。
...
| `needs_user` | workflow 进入 `waiting_user`，写入 `pending_question`。 | 不派发后续节点。 |
...
1. 如果 workflow `status` 是 `waiting_user`，返回用户问题，不派发节点。
2. 如果存在 running node，先等待或由用户决定 cancel/retry。
3. 如果有 failed/blocked node，按 workflow policy 判断能否重试或需要用户决策。
4. 如果 task graph 有 runnable task，派发 implementer。
5. 如果所有 task graph task 达到 task-level passed，派发 finish 或确认 workflow finished。
6. 如果没有 task graph 且还处于入口阶段，才回到 workflow entrypoint。
