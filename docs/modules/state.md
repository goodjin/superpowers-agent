...
插件进程启动时从 durable snapshot 加载 state，然后先执行 reconciliation，再初始化 runtime memory。任何从 durable 恢复出的 `node_runs[].status === "running"` 都不能原样进入当前事实；它必须被转成 `interrupted`，workflow 顶层进入 `recovered_unknown`，等待用户决定重试、取消或检查。
