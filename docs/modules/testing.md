...
- `test/tui-plugin.test.ts`：TUI route、命令入口、resident progress slot 注册和无 session props compact progress 渲染。
...
- `test/tui-plugin.test.ts` 覆盖 `superpowers-progress` route、`superpowers.progress` 命令入口、resident progress slot 名单，以及 parent/child/no-props/unrelated session 下的 compact progress 行。
