...
主会话常驻进度通过 TUI slot 展示，但不注册到 prompt/input 邻近区域，避免启动或刷新时挤占输入框和底部操作元素：
...
- `session_prompt_right`、`home_prompt_right`、`home_footer`：不再注册 Superpowers compact progress。前两个会靠近输入区，容易挤占输入框；`home_footer` 与 host footer 区域语义不稳定，先交给主会话底部和 sidebar surface 承载。
