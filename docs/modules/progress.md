...
- `src/tui.ts`：注册 `superpowers-progress` TUI route 和 `superpowers.progress` 命令入口。
...
TUI entry 暴露在 package 的 `./tui` export。面板 route 名为 `superpowers-progress`，命令值为 `superpowers.progress`。面板展示 active run、每个 node 的 agent、phase、task、session、durable status、live session status 和最新进展摘要。
