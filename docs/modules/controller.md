...
- `sp_start`：在用户确认后启动 workflow。对新 run，它创建 active run 并派发入口节点；对 prepared run，它激活 draft 并派发下一步；对已存在 run 的恢复，它必须根据当前 state 恢复到正确下一步，而不是无条件回到入口。对 `waiting_user` run，只有带 `resume_input` 的 `sp_start(run_id, resume_input)` 才会把用户回答传回原 child session。对 `recovered_unknown` run，普通 `sp_start(run_id)` 只返回恢复提示；用户确认重试后，`sp_start(run_id, task_id)` 为对应 interrupted task 创建新 attempt。`sp_start` 派发或恢复 child prompt 后返回，不等待 child session 完整跑完。
