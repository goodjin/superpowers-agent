...
- `src/tools/sp-prepare.ts`：创建 prepared workflow；v4 中可按 `prepare_mode` 派发 managed design 或 managed planning draft node。
- `src/tools/sp-start.ts`：确认后创建 active run，批准 design/plan candidate，激活 proposal-only draft，恢复已有 run，或用 `resume_input` 恢复等待用户输入的 child session。
...
- `sp_status`：读取当前 active workflow、等待用户输入的 workflow、未完成历史 workflow 和 task/session 状态。它只读，不改变 runtime 状态。当前事实以 runtime memory 为准；durable 文件只作为恢复快照和审计材料出现在响应的 `durable` 区块。
- `sp_prepare`：创建或更新一个 `draft` run，写入 request/proposal/state。`proposal_only` 不派发 child node；`managed_design` 会派发 `sp-designer` 产出 candidate design；`managed_planning` 会派发 `sp-planner` 产出 candidate plan/task graph。
- `sp_start`：在用户确认后启动 workflow。对新 run，它创建 active run 并派发入口节点；对 prepared run，它根据 `start_action` 批准 design、批准 plan、启动 proposal、恢复用户输入或重试节点。对 `waiting_user` run，只有带 `resume_input` 的 `sp_start(run_id, resume_input)` 才会把用户回答传回原 child session。对 `recovered_unknown` run，普通 `sp_start(run_id)` 只返回恢复提示；用户确认重试后，`sp_start(run_id, task_id)` 或 `start_action=retry_node` 为对应 interrupted task 创建新 attempt。`sp_start` 派发或恢复 child prompt 后返回，不等待 child session 完整跑完。
...
- `controller_feedback`：public tool 返回给 controller 的结构化下一步，包含 recommended next、approval target、blocking reason 和 artifact mode。
...
3. `sp_prepare` 创建 `draft` run，写入 task/proposal/state 文件。feature 默认走 `managed_design` 并派发 designer draft；plan-only 默认走 `managed_planning` 并派发 planner draft；轻量任务可显式使用 `proposal_only`。
...
8. `sp_start` 激活 prepared run 或恢复已有 run，并根据 workflow kind、entrypoint、current phase、task graph 和 node_runs 派发下一步；它只等待必要的 session 创建和 node registration，不等待 child session turn 完成。
9. 节点会话完成或需要追加中间结果时调用 `sp_report`。
10. runtime 根据 transition 规则派发后续节点，直到 finish、waiting_user、blocked、failed 或 canceled。后续派发同样是 prompt 调度后返回，不能把上游 `sp_report` 卡到下一个 child session 完成。
...
- 如果 plugin startup 把旧 running node 标成 `interrupted` 并把 workflow 置为 `recovered_unknown`，`sp_start(run_id)` 不能自动派发。主控应先向用户确认，再用 `sp_start(run_id, task_id)` 重试指定 interrupted task，或用 `sp_cancel` 取消。
...
- dispatch 创建失败会登记 `dispatch_failed` node，并让 workflow 进入 `waiting_user_decision`，`controller_feedback` 给出 retry/cancel 选择。
