...
- `sp_start`：在用户确认后启动 workflow。对新 run，它创建 active run 并派发入口节点；对 prepared run，它激活 draft 并派发下一步；对已存在 run 的恢复，它必须根据当前 state 恢复到正确下一步，而不是无条件回到入口。
...
- `needs_user` 由节点通过 `sp_report` 上报，super-agent 读取 `sp_status` 后向用户提问或执行用户选择。节点 agent 不能绕过 runtime 调 OpenCode 原生 question。
