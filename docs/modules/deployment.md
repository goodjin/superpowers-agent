...
- `scripts/install.sh`: user-facing installer. It checks Bun, warns when `opencode` is missing, delegates config mutation to the TypeScript CLI installer, runs `doctor`, and prints next steps.
