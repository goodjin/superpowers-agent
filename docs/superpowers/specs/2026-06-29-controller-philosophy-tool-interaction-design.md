...
Generate a workflow_spec when starting new work. Each node must have an agent, a scoped task, required context, expected output, report_contract, and transition condition. Include document contracts when project or workflow documents must be produced, such as docs/features, docs/bugfix, docs/spec.md, docs/plan.md, or docs/modules updates. Ask the user to confirm workflow execution when the workflow changes files, runs high-risk commands, or the project instructions require confirmation.
...
v5 需要把文档作为 workflow spec 的显式 contract，而不是让节点随手写文件。文档分两类：
...
- workspace/user documents: node agent 生成或修改，给用户和项目使用，例如 `docs/features/*.md`、`docs/bugfix/*.md`、`docs/spec.md`、`docs/plan.md`、`docs/modules/*.md`。
...
node 通过 `consumes` 和 `produces` 引用 document id。插件校验这些引用，但不替 controller 决定哪些文档应该存在。

### 6.2 生成时机

- controller intake: 只在主会话中问清需求和展示草案。用户未批准前，可以只输出给用户查看，不必写入 workspace 文档。
- `sp_prepare(register_workflow)`: 插件写入 `workflow-spec.json`、`documents.json`、draft state 和 `events.jsonl`。这是运行时文档，不是 feature/bugfix 交付文档。
- `sp_start`: 插件派发 node 前生成 `nodes/<node-id>/task.md`；如果有 `task_id`，同时生成 `reports/<task-id>/task.md`。
- designer/planner/documentation node: 根据 workflow spec 创建 `docs/features/*.md`、`docs/bugfix/*.md`、`docs/spec.md` 或 `docs/plan.md` 等 workspace 文档，并通过 `sp_report` 汇报路径、摘要和必要 artifact。
- implementer/reviewer/verifier node: 使用 canonical 的 spec/plan/task 文档作为输入，输出 task-scoped report、patch summary、check evidence。
- finisher/documentation node: 如果实现改变模块契约，负责在 finish 前更新 `docs/modules/*.md`，并通过 `sp_report` 记录更新。

### 6.3 candidate 与 canonical

- `progress` report 产生的文档只能是 candidate/progress，不能解锁下游。
- `passed` report 可以让 `promotion="on_node_passed"` 的文档成为 canonical。
- `promotion="on_controller_approval"` 的文档必须等待 controller 明确批准后才能成为 canonical。
- 下游 node 只能消费已经 canonical、且在 `consumer_node_ids` 中允许它消费的文档。
- 如果 required 文档缺失、stale 或没有 canonical，transition 应返回 controller decision 或 blocked，不能让 node 自行搜索替代材料。

常见例子：

- 本仓库 feature/bugfix 规则要求实现前先有 `docs/features/` 或 `docs/bugfix/` 文档。controller 应在 workflow spec 中把该文档声明为 implementation node 的 required input。
- `sp-designer` 产出的 spec 可以先作为 candidate 给用户审阅；批准后成为 planner 的 canonical input。
- `sp-planner` 产出的 plan/task graph 批准后，`task_graph` 进入 runtime，`plan` 文档成为 implementer、reviewer、verifier 的 canonical input。
...
- workflow 文档通过 `documents` contract 关联 node，生成时机和 promotion 规则明确。
