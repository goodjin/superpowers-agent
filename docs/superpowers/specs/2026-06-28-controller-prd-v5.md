...
v5 的变化集中在一点：插件不再把内置 workflow kind 当作主决策来源。controller 根据用户需求生成一个简单的 bootstrap `workflow_spec`；plugin 校验、持久化并执行；node agent 只执行当前 node，并通过 `sp_report` 交回结构化结果。执行中的 agent 可以在 report 中产出新任务或 workflow expansion，plugin 在校验通过后自动追加到 workflow 并继续执行。

```text
controller 生成 bootstrap workflow_spec
plugin 校验并执行 workflow_spec
node agent 执行单个 node 并调用 sp_report
agent report 可产生新任务或 workflow expansion
plugin 根据 workflow_spec + result + expansion_policy 推进或反馈 controller
```
...
- 根据 Superpowers 工作理念和常用 workflow 示例生成本次 bootstrap `GeneratedWorkflowSpec`。
- 设置 `expansion_policy`，决定 node report 生成的新任务是否允许自动扩展到 workflow。
...
- 收集和校验 `sp_report`。
- 根据 `WorkflowState`、`GeneratedWorkflowSpec`、node result 和 `expansion_policy` 计算下一步。
...
| `passed` | 关闭 node，校验 artifacts/gates/report contract；如果包含 task graph 或 workflow expansion，则按 expansion policy 校验并追加。 | expansion 合法时自动派发新增 runnable node；否则按 workflow spec edge 推进。 |
...
-> super-agent generates bootstrap workflow_spec
...
1. controller 能读取 agent catalog、schema 和 workflow examples，但只需要生成 bootstrap workflow spec。
...
7. planner `sp_report(status="passed", task_graph=...)` 时，如果 expansion policy 为 auto，plugin 自动把任务扩展成 execution/check/finish nodes 并继续派发。
8. `expansion_policy.mode="disabled"` 时，plugin 不应用 planner 产出的 task graph 或 workflow expansion，只保存为 artifact。
9. `sp_report(status="passed")` 无 expansion 时按 workflow spec edge 自动推进或返回 controller decision。
10. `sp_report(status="failed")` 只有在 spec 有明确 failure edge 时自动推进。
11. `sp_report(status="needs_user")` 写入 pending question，并通知 parent controller session。
12. `sp_start(resume_input)` 恢复原 child session，不创建新 node。
13. child session 无 terminal report 时生成 fallback summary，并进入 controller decision。
14. `spec.md`、`plan.md` 等 workflow artifacts 只从 run 目录读取并内联给 node agent。
15. TUI 能展示动态 node graph、fallback、attention 和 progress digest。
16. startup recovery 不把 durable running 当成 live running。
17. public tool surface 不新增工具，仍是 `sp_status`、`sp_prepare`、`sp_start`、`sp_cancel`、`sp_report`。

## 13. Migration Notes From V4

| V4 concept | V5 replacement |
|---|---|
| fixed workflow kind decides dispatch | controller-generated bootstrap spec plus agent-generated expansion decides dispatch |
